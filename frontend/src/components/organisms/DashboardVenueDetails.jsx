...
const VenueDetails = ({ is_open, on_close, day_data, current_venue_data, all_venues_data, favorite_venues_data, on_select_venue }) => {
...
const VenueDetailsSection = ({ 
  ...
  return (
    <div className="venue-details-section-wrapper">
      <div className="venue-details-section-header" onClick={handle_click} style={{ cursor: 'pointer' }}>
        <h3>{title}</h3>
      ...
      {!is_link && (
        <div className={`venue-details-section-content-wrapper ${is_expanded ? 'open' : ''}`}>
          <div className="venue-details-section-content-inner">
            {children}
          </div>
        </div>
      )}
